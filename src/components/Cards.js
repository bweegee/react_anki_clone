...
class EditCards extends React.Component {
  ...
  render() {
    const rows = this.props.cards.map((card, i) => {
      return (
        <tr key={i}>
          <td contenteditable="true">{card.front}</td>
          <td contenteditable="true">{card.back}</td>
          <td contenteditable="true">{card.learning}</td>
          <td>
            <Button
              data-index={i}
      ...
      <div>
        ...
        <Form>
          <Form.Group widths="equal">
            ...
            />
            <Form.Button onClick={this.addCard} color="green" size="tiny">
              Add Card
            </Form.Button>
          </Form.Group>
        </Form>
